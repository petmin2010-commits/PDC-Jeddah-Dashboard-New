...
if(!lastRows.length){host.classList.add('is-empty');track.innerHTML='<span>لا توجد ملاحظات جديدة خلال آخر 72 ساعة — التحليل يعتمد على الشيتات والداشبورد فقط.</span>';if(btn)btn.style.display='none';return;}
host.classList.remove('is-empty');if(btn)btn.style.display='';const body=lastRows.map(item).join('');track.innerHTML=body+body;const dur=Math.max(30,Math.min(140,lastRows.length*8));track.style.setProperty('--news-duration',dur+'s');}
